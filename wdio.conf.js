...
exports.config = {
    ...
    onPrepare: function (config, capabilities) {
        ...
        let nonBuiltInReturnCommands = [
            ...
            'dragAndDrop', 'moveTo', 'scrollIntoView',
            'waitForDisplayed', 'waitForClickable', 'waitForExist', 'waitForEnabled',
            'waitUntil'
        ];
        let builtInReturnCommands = [
        ...
        for(let cmd of builtInReturnCommands) {
            Object.defineProperty(Promise.prototype, cmd, {
                enumerable: false,
                value: async function(...args) {
                    let elem = await this;
                    return await elem[cmd](...args);
                }
            });
        }
        ...
        Object.defineProperty(Array.prototype, 'getDisplayed', {
            ...
            value: async function(timeout = 60000) {
                return await browser.waitUntil(async () => {
                    ...
                    if (elems.length > 0) {
                        const mappedArray = await Promise.all(elems.map(elem => {
                            return elem.isDisplayed();
                        }));
                        let displayableElems = elems.filter((value, index) => mappedArray[index]);
                        if(displayableElems.length){
                            return displayableElems;
                        } 
                    }
                    return false;
                }, {timeout});
